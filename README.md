# FinCast — AI-Powered Stock Market Platform

## Hackathon Submission: AI for Bharat

This repository contains the requirements and design documentation for FinCast, an AI-powered stock market platform submitted for the AI for Bharat hackathon.

---

##  Live MVP

 **[https://fincast.web.app/](https://fincast.web.app/)**

---

##  Repository Contents

| File | Description |
|------|-------------|
| [requirements.md](requirements.md) | Detailed product requirements — user stories, acceptance criteria, features, non-functional requirements, risk assessment, and competitive analysis |
| [design.md](design.md) | Comprehensive system design — architecture, database schema, API design, ML pipeline, UI/UX wireframes, security, performance, and deployment |

---

## What is FinCast?

**FinCast** is an AI-powered stock market intelligence platform designed to help investors make data-driven decisions through machine learning, real-time analytics, and automated insights.

### Key Features

- **AI Stock Recommendations** — LSTM + XGBoost ensemble model enhanced with FinBERT sentiment analysis to identify high-potential stocks.

- **Real-Time Market Data** — Live charts integrated with technical indicators such as RSI, MACD, and SMA for advanced market analysis.

- **ML Price Predictions** — Forecasted closing prices with confidence scores and built-in risk assessment.

- **Smart Alerts** — FCM push notifications with automated position-closing suggestions based on ML predictions.

- **Portfolio Management** — Track holdings, profit & loss, and sector allocation with Angel One integration.

- **News Sentiment Analysis** — FinBERT-powered sentiment evaluation of financial news to gauge market mood.

---

##  Tech Stack

| Layer | Technology |
|-------|------------|
| Frontend | React + TypeScript, TradingView Charts |
| Backend | Node.js + Express, FastAPI (ML) |
| Database | MongoDB Atlas, Redis Cloud |
| ML Models | LSTM, XGBoost, FinBERT |
| Notifications | Firebase Cloud Messaging (FCM) |
| Payments | Razorpay |
| Broker API | Angel One SmartAPI |
| Hosting | Firebase Hosting |

---

## 👥 Team

- **Manimaran K** — Team Leader  
- **Navin S** — Team Member  
- **Pavan K** — Team Member  
- **Nitin T** — Team Member  

Built with for the AI for Bharat hackathon.
